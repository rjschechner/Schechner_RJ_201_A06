...
[https://github.com/rjschechner/Schechner_RJ_201_A06](https://github.com/rjschechner/Schechner_RJ_201_A06)
...
for(player in names(player_info)) {
  player_info[[player]]$height <- unname(heights[player])
}
...
```{r}
points_rounded <- lapply(points_per_game, round, digits =2)
...
rounding_matrix <- apply(mat_rand, MARGIN = c(1,2), FUN = rounding_function)
print(rounding_matrix)
```
...
```{r}
weather_data <- max_temps[, -(1:2)]
average <- colMeans(weather_data)
print(average)
```
...
colnames(CO)
print(CO)
monthly_temps <- CO[, 3:14]
median <-apply(monthly_temps, 1, median, na.rm = TRUE)
overall_median <- median(median, na.rm = TRUE)
(print(overall_median))

max_index <- which.max(median)

min_index <- which.min(median)

year_max_temp <- CO$year[max_index]
year_min_temp <- CO$year[min_index]
max_temp <- median[max_index]
min_temp <-median[min_index]

paste0("The maximum median annual temperature of ", max_temp, " occurred in ", year_max_temp, " and the minimum median annual temperature of ", min_temp, " occurred in ", year_min_temp, ".")
